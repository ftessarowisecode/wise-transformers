...
# setting device via env varibles
# if gpu training
os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
os.environ["CUDA_VISIBLE_DEVICES"]= "0"


def print_report(report, d=4, out_file=None):
    """store a classification report for KPI tracking
    ...
    with open(out_file, mode="w") as file:
        # header
        file.write('\tlabel\tprecision\trecall\tf1-score\tsupport\n')

        #remove the bottom 3 for now
        for c in list(report.keys())[:-3]:
            file.write(f"\t{c}\t{report[c]['precision']:.{d}}\t{report[c]['recall']:.{d}}\t{report[c]['f1-score']:.{d}}\t{int(report[c]['support'])}\n")
        ...
        c = 'weighted avg'
        file.write(f"\t{c}\t{report[c]['precision']:.{d}}\t{report[c]['recall']:.{d}}\t{report[c]['f1-score']:.{d}}\t{int(report[c]['support'])}\n")
        file.write(f"\taccuracy\t{report['accuracy']:.{d}}\n")
...
class DataTrainingArguments:
    ...
    label_column_name_l1: Optional[str] = field(
        default=None,
        metadata={
            "help": (
    ...
    label_column_name_l2: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "The name of the label column in the input dataset or a CSV/JSON file. "
    ...
    batch_size: int = field(
        default=32, metadata={"help": "Batch size for the training"}
    )
    epochs: int = field(
        default=5, metadata={"help": "num epochs"}
    )
    ...
    add_extra_tokens: Optional[int] = field(
        default=None, metadata={"help": "Include UNK more frequent tokens to the tokenizer."}
    )
...
def get_label_list(raw_dataset, split="train") -> List[str]:
    ...
    # else:
    label_list_l1 = raw_dataset[split].unique("label_l1")
    label_list_l2 = raw_dataset[split].unique("label_l2")

    # we will treat the label list as a list of string instead of int, consistent with model.config.label2id
    label_list_l1 = [str(label) for label in label_list_l1]
    label_list_l2 = [str(label) for label in label_list_l2]

    return label_list_l1, label_list_l2
...
def main():
    ...
    if data_args.label_column_name_l1 is not None and data_args.label_column_name_l1 != "label_l1":
        for key in raw_datasets.keys():
            raw_datasets[key] = raw_datasets[key].rename_column(data_args.label_column_name_l1, "label_l1")
    ...
    if is_regression:
        logger.info("No Regression")
    else:  # classification
        if raw_datasets["train"].features["label_l1"].dtype == "list":  # multi-label classification
            is_multi_label = True
        ...
        label_list_l1, label_list_l2 = get_label_list(raw_datasets, split="train")
        # label_list_l2 = get_label_list(raw_datasets, split="train")

        for split in ["validation", "test"]:
            if split in raw_datasets:
                val_or_test_labels_l1, val_or_test_labels_l2  = get_label_list(raw_datasets, split=split)
                logger.info(val_or_test_labels_l1)
                logger.info(label_list_l1)
                diff1 = len(set(val_or_test_labels_l1).difference(set(label_list_l1)))
                diff = diff1 + len(set(val_or_test_labels_l2).difference(set(label_list_l2)))
        ...
        label_list_l1.sort()
        label_list_l2.sort()
        num_labels_l1, num_labels_l2 = len(label_list_l1), len(label_list_l2)
        if num_labels_l1 <=1 or num_labels_l2 <= 1:
    ...
    #     finetuning_task="multi_taxonomy_classification",
    #     cache_dir=model_args.cache_dir,
    ...
    config = DistilBertForMultiLabelConfig(
        # model_args.config_name if model_args.config_name else model_args.model_name_or_path,
        finetuning_task="multi_taxonomy_classification", 
        num_classes_per_level=[num_labels_l1, num_labels_l2]
    )
    logger.info(config)
    config.problem_type = "multi_taxonomy_classification"
    ...
    if data_args.add_extra_tokens:
        logger.info(f"Extending tokenizer vocab with UNK tokens with freq > {data_args.add_extra_tokens}")
        # Define the word frequency threshold
        threshold = data_args.add_extra_tokens

        # Create a dictionary to store word counts
        word_counts = {}
        text_columns = data_args.text_column_names.split(",")

        # Iterate through the text column and count words
        for sentence in raw_datasets["train"][text_columns[0]]:
            # print(sentence)
        ...
        logger.info(f"number of selected selected words = {len(selected_words)} from train-set with occurrence > {threshold}")

        not_in_bert = []
        # Check if each word is in the tokenizer
        for word in selected_words:
            if word not in tokenizer.get_vocab():
                # If not in the tokenizer, add it
                logger.debug(f"Adding token: {word}")
                tokenizer.add_tokens(word)
                not_in_bert.append(word)
                
        # If you want to add the word to the model as well, you need to resize the model's embeddings
        model.resize_token_embeddings(len(tokenizer))
        logger.info(f"Added {len(not_in_bert)} new tokens to model and tokenizer!")
    ...
    if training_args.do_train and not is_regression:  # classification, training
        logger.info(f"labels L1: {label_list_l1}")
        logger.info(f"labels L2: {label_list_l2}")
        label_to_id_l1 = {v: i for i, v in enumerate(label_list_l1)}
        label_to_id_l2 = {v: i for i, v in enumerate(label_list_l2)}
        # update config with label infos
        if model.config.label2id_l1 != label_to_id_l1:
            logger.warning(
        ...
        model.config.label2id_l1 = label_list_l1
        model.config.label2id_l2 = label_list_l2
        # model.config.id2label = {id: label for label, id in config.label2id.items()}
        # logging.info(f"debug: {model.config.id2label}")
    elif not is_regression:  # classification, but not training
        logger.info("using label infos in the model config")
        # logger.info("label2id: {}".format(model.config.label2id))
        # label_to_id = model.config.label2id
        # model.config.id2label = {id: label for label, id in config.label2id.items()}
        # logger.info("******** debug: {}".format(model.config.id2label))
    else:  # regression
        label_to_id = None
    ...
    def preprocess_function(examples):
        ...
        if label_to_id_l1 is not None and "label_l1" in examples:
            result["label_l1"] = [(label_to_id_l1[str(l)] if l != -1 else -1) for l in examples["label_l1"]]
        if label_to_id_l2 is not None and "label_l2" in examples:
            result["label_l2"] = [(label_to_id_l2[str(l)] if l != -1 else -1) for l in examples["label_l2"]]
        # combine labels in a single tuple
        result["labels"] = [[lb1, lb2] for lb1, lb2 in zip(result["label_l1"], result["label_l2"]) ] #
        return result
    ...
    training_args.per_device_eval_batch_size=data_args.batch_size
    ...
    logger.info(f"*** best metric for the model: {training_args.metric_for_best_model} macro")
    ...
    # if training_args.do_predict:
        ...
        #     labels = [label_list[lb_id] for lb_id in labels]
        #     predict_dataset = predict_dataset.remove_columns("label")
        # text = predict_dataset["sentence"]
        ...
        #         writer.write("index\ttext\tlabel\tprediction\n")
        ...
        #                 writer.write(f"{index}\t{text[index]}\t{labels[index]}\t{item}\n")
        #     logging.info("Classification report")
        #     report = classification_report(labels, [label_list[item] for item in predictions], 
        #                                    digits=4, output_dict=True)   
        #     print_report(report, out_file=os.path.join(training_args.output_dir, "classification_report.txt")) 
